...
export default function Header({ navigation }: { navigation: NavigationItem[] }) {
  ...
  //const NavLogo = () => <img className='h-12 w-26' src={logo} alt='Your SaaS App' />;
  const NavLogo = () => <img className='h-30 w-30 rounded-full' src={logo} alt='Your SaaS App' />;
    ...
    <header className='absolute inset-x-0 top-0 z-50 dark:bg-boxdark-2'>
      <nav className='flex items-center justify-between p-6 lg:px-8' aria-label='Global'>
        <div className='flex items-center lg:flex-1'>
          ...
          >
            <NavLogo />
            <span className='ml-2 text-sm font-semibold leading-6 dark:text-white'>swiftshare</span>
          </a>
        </div>
